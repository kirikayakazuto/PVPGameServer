...
export default class loginBonues {
    ...
    public static getLoginBonuesData() {
    ...
    public static async getLoginBonuesInfo(player: AreaPlayer) {
        ...
        if(!arr || arr.length <= 0) return {code: RES.ERR_SYSTEM, msg: "没有获取到签到信息!"};

        let bonuesInfo = arr[0];
        if(bonuesInfo.status != 0) return {code: RES.GET_LOGIN_BONUES_AGO, msg: {
            isSign      : true,
            bonues      : 0,
            days        : bonuesInfo.days,
            allBonues   : GameServerConfig.loginBonues.bonues
        }};
